...
class CheckoutController extends Controller
{
    ...
    public function checkout(ApiCheckoutCartRequest $request, $customerId)
    {
        $cartItems = Cart::where('customer_id',$customerId)->orWhere('ip_address', $request->ip())->get();
        $roders=[];
        $count =0;
        foreach($cartItems as $cart){
            ...
            $roders[] = new OrderResource($order);
            $count++;
        }
        return $roders; 
    }
